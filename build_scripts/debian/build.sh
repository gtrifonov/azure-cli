...
for d in $source_dir/src/azure-cli $source_dir/src/azure-cli-core $source_dir/src/azure-cli-nspkg \
  ...
  do cd $d;
  sudo python3 setup.py bdist_wheel -d $tmp_pkg_dir;
  cd -;
done;
all_modules=`find $tmp_pkg_dir -name "*.whl"`
sudo pip3 install $all_modules
sudo pip3 install --force-reinstall --upgrade azure-nspkg azure-mgmt-nspkg
